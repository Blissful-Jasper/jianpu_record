...
class model_pred:
    def __init__(self,sat_path,  scaler_x_path, model_path, scaler_y_path, output_path):
    ...
    def predict(self):
        ...
        ds = self.sat_data.tbb.sel(lat=slice(24,36.79),
                                  lon=slice(116,128.79))
        time = ds.time.values
        ...
        ds_smooth = mpcalc.smooth_gaussian(ds_pred.data, 8).values

        ds_all = xr.Dataset({'precip': ([ 'latitude','longitude','time', ], ds_smooth)},
                            coords={'time': new_time,
                                    'longitude': ds_pred.lon.data,
                                    'latitude': ds_pred.lat.data,
...
def main(sat_path, scaler_x_path, model_path, scaler_y_path, output_path):
        
    predicter = model_pred(sat_path, scaler_x_path, model_path, scaler_y_path,output_path)
    predicter.predict()
...
if __name__ == "__main__":
    ...
    main(args.sat_path, args.scaler_x_path, args.model_path, args.scaler_y_path, args.output_path)
